...
class DataManager:
    ...
    _adjust_columns: List[str] = ["foreAdjustFactor", "backAdjustFactor", "adjustFactor"]
    _fields: List[str] = ["date", "open", "high", "low", "close", "preclose", "volume", "amount", "turn", "tradestatus", "pctChg", "peTTM", "psTTM", "pcfNcfTTM", "pbMRQ", "isST"]
    ...
    def _load_all_a_shares_base(self) -> None:
        self._all_a_shares = []  # a bug in the all a shares code init
        if os.path.exists(self._a_shares_list_path):
            ...
            self._all_a_shares = [f"{stk_id[:2].lower()}.{stk_id[-6:]}" for stk_id in all_a_shares]

    def _load_all_a_shares(self):
        self.logger.info("Loading A-Shares stock list")
        self._login_baostock()
        self._load_all_a_shares_base()
        qry = bs.query_all_stock(day=str(datetime.date.today()))
        stocks = {code for code in self._result_to_data_frame(qry)["code"] if code.startswith("sh") or code.startswith("sz")}
    ...
    def _parallel_foreach(self, callable, input: List[dict], max_workers: Optional[int] = None) -> list:
    ...
    def _fetch_basic_info(self) -> pd.DataFrame:
        self.logger.info("Fetching basic info")
        dfs = self._parallel_foreach(self._fetch_basic_info_job, [dict(code=code) for code in self._all_a_shares])
        df = pd.concat(dfs)
    ...
    def _fetch_adjust_factors(self) -> pd.DataFrame:
        ...
        self.logger.info("Fetch adjust factors")
        dfs: List[pd.DataFrame] = self._parallel_foreach(
            self._fetch_adjust_factors_job, [dict(code=code, start=one_year_before_ipo(data["ipoDate"])) for code, data in self._basic_info.iterrows()]
        )
    ...
    def _adjust_factors_for(self, code: str) -> pd.DataFrame:
        ...
        if code not in adj_factor_idx:
            ...
            return pd.DataFrame([[1.0, 1.0, 1.0]], index=pd.Index([start]), columns=self._adjust_columns)
    ...
    def _download_stock_data_job(self, code: str, data: pd.Series) -> None:
        ...
        self._login_baostock()
        query = bs.query_history_k_data_plus(code, fields_str, start_date=data["ipoDate"], adjustflag="2")
        ...
        df[self._adjust_columns] = df[self._adjust_columns].ffill().fillna(1.0)
    ...
    def _download_stock_data(self) -> None:
        self.logger.info("Download stock data")
        os.makedirs(f"{self._save_path}/k_data", exist_ok=True)
        self._parallel_foreach(self._download_stock_data_job, [dict(code=code, data=data) for code, data in self._basic_info.iterrows()])
    ...
    def _save_csv(self) -> None:
        self.logger.info("Export to csv")
        children = list(Path(f"{self._save_path}/k_data").iterdir())
        self._parallel_foreach(self._save_csv_job, [dict(path=path) for path in children])
    ...
    def _dump_qlib_data(self) -> None:
        DumpDataAll(csv_path=self._export_path, qlib_dir=self._qlib_export_path, max_workers=self._max_workers, exclude_fields="date,code", symbol_field_name="code").dump()
        shutil.copy(f"{self._qlib_export_path}/calendars/day.txt", f"{self._qlib_export_path}/calendars/day_future.txt")
    ...
    def fetch_and_save_data(
        ...
        use_cached_stock_data: bool = False,  # New parameter
        use_cached_csv_data: bool = False,  # New parameter
    ):
        self._load_all_a_shares()

        if use_cached_basic_info:
            basic_info_path = Path(f"{self._save_path}/basic_info.csv")
            if basic_info_path.exists():
                self.logger.info("Using cached basic info.")
                self._basic_info = pd.read_csv(basic_info_path, index_col=0)
            else:
                self.logger.warning("Cached basic info not found. Fetching...")
                self._basic_info = self._fetch_basic_info()
        else:
            self._basic_info = self._fetch_basic_info()

        if use_cached_adjust_factor:
            adjust_factors_path = Path(f"{self._save_path}/adjust_factors.csv")
            if adjust_factors_path.exists():
                self.logger.info("Using cached adjust factors.")
                self._adjust_factors = pd.read_csv(adjust_factors_path, index_col=[0, 1])
            else:
                self.logger.warning("Cached adjust factors not found. Fetching...")
                self._adjust_factors = self._fetch_adjust_factors()
        else:
            self._adjust_factors = self._fetch_adjust_factors()

        # Conditional download of stock data
        if not use_cached_stock_data:
            self._download_stock_data()
        else:
            # Check if k_data directory exists and is not empty
            k_data_dir = Path(f"{self._save_path}/k_data")
            if k_data_dir.exists() and any(k_data_dir.iterdir()):
                self.logger.info("Using cached stock data (pkl files).")
            else:
                self.logger.warning("Cached stock data (pkl files) not found or directory is empty. Downloading...")
                self._download_stock_data()

        # Conditional saving of CSV data
        if not use_cached_csv_data:
            self._save_csv()
        else:
            # Check if export directory (for CSVs) exists and is not empty
            export_dir = Path(self._export_path)
            if export_dir.exists() and any(export_dir.iterdir()):
                self.logger.info("Using cached CSV data.")
            else:
                self.logger.warning("Cached CSV data not found or directory is empty. Saving CSVs...")
                self._save_csv()
        self.logger.info("Start dump Qlib data")
        self._dump_qlib_data()


if __name__ == "__main__":
    today = str(datetime.date(2025, 5, 14))
    logger = AlphaLogger()
    logger.info(f"Forward adjust date: {today}")
    dm = DataManager(
        save_path="./data/tmp",
    ...
    #     forward_adjust_date=today,
    # )
    # dm.fetch_and_save_data(use_cached_basic_info=True, use_cached_adjust_factor=True, use_cached_stock_data=True, use_cached_csv_data=True)
