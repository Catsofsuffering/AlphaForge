...
class Builders:
    ...
    def add_token(self, tokens):
        ...
        for i in range(self.batch_size):
            ...
            if not self.builders_done[i]:
                ...
                if isinstance(token, SequenceIndicatorToken) and token.indicator == SequenceIndicatorType.SEP:
        ...
        for cur in to_iter:
            ...
            if cur is not None:
                try:
                    ...
                    ic = 0.0
                    multi_score = {}
                    ret_tensor = np.array([0.0] * data.n_days)
                if np.isnan(ic):
                    ic = 0.0
                    multi_score = {}
                    ret_tensor = np.array([0.0] * data.n_days)
    ...
    def build_exprs(self):
        res = [self.builders[i].get_tree() if self.builders[i].is_valid() else None for i in range(self.batch_size)]
        self.exprs = res
    ...
    def _valid_action_types(self, builder) -> dict:
        ...
        ret = {
            ...
            "op": {UnaryOperator: valid_op_unary, BinaryOperator: valid_op_binary, RollingOperator: valid_op_rolling, PairRollingOperator: valid_op_pair_rolling},
        }
        return ret
