...
class NetG_Lstm(nn.Module):
    def __init__(
        ...
        self.fc_h = nn.Sequential(nn.Linear(potential_size, n_layers * d_model), nn.ReLU())
        self.fc_c = nn.Sequential(nn.Linear(potential_size, n_layers * d_model), nn.ReLU())
        self.emb = nn.Embedding(n_chars + 1, d_model, 0)
        self.rnn = nn.LSTM(input_size=d_model, hidden_size=d_model, num_layers=n_layers, batch_first=True, dropout=dropout)
...
def train_network_generator(netG, netM, netP, cfg, data, target, current_round, random_method, metric, lr, n_actions):
    ...
    for epoch in range(cfg.num_epochs_g):
        ...
        if cfg.g_es_score == "mean":
            ...
            es_score = max_score + 2.0 * std_score
        else:
